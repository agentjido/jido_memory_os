defmodule Jido.MemoryOS.Actions.PostTurn do
  @moduledoc """
  Action wrapper for framework adapter `post_turn/3`.
  """
  ...
  use Jido.Action,
    name: "memory_os_post_turn",
    description: "Run framework adapter post_turn and write result into state",
    schema: [
      framework_adapter: [
        type: :any,
        required: false,
      ...
      turn_output: [type: :any, required: false, doc: "Optional explicit post-turn payload"],
      ...
      memory_result_key: [type: :any, required: false, doc: "Result key for post-turn payload"]
    ]
  ...
  def run(params, context) do
    ...
    with {:ok, adapter} <- AdapterSupport.resolve_adapter(map_params, context, :post_turn),
         {:ok, result} <-
           adapter.post_turn(
             target,
             payload,
      ...
      key = map_get(map_params, :memory_result_key, :memory_post_turn)
      {:ok, %{key => result}}
    else
      {:error, reason} ->
        ...
        {:error, AdapterSupport.normalize_adapter_error(adapter, reason, :post_turn)}
    end
  end
